...
在直接 BGRA 优化之前，一次同场景、每段 20 秒的对比中，原生录制约 173.56 FPS，旧图片序列流程约 162.42 FPS；停止保存耗时分别为 1.10 秒和 8.90 秒。原生录制纹理显存估算为 79.1 MiB，旧流程为 23.7 MiB。这是简单场景实测，两种编码器的参数不同，不是同画质压缩效率评测，也不代表此次优化后的性能数据。同尺寸直接采集减少一张画面大小的 RGBA 纹理，1920×1080 时约为 7.9 MiB；实际耗时收益尚未测量。CPU 数据、声画时间与限制见[验证报告](Native~/VALIDATION.md)。
