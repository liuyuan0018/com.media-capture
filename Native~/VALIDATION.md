...
In the existing Play session, a frame-end probe captured the same 1920×1080 Game View through the previous RGBA-capture/BGRA-blit path and through `Graphics.Blit(null, BGRA)`. All 2,073,600 pixels had identical RGB values: zero differing pixels, zero mean channel error and zero maximum RGB-sum error. The reference channel range was 0–255. Direct BGRA capture through the screenshot API also matched with both disabled and enabled `GL.sRGBWrite`; the implementation uses the direct Blit path with sRGB writes disabled. Temporary textures and the probe object were scheduled for destruction after the comparison. CPU pixel readback was used only by this diagnostic probe.
